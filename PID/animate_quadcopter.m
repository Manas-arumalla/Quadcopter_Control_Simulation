%% File: animate_quadcopter.m

function animate_quadcopter(X, params, time, Ref)
    figure('Name','Quadcopter animation');
    axis equal; grid on; hold on;
    xlabel('X [m]'); ylabel('Y [m]'); zlabel('Z [m]');
    view(45,20);
    plot3(Ref(1,:), Ref(2,:), Ref(3,:), '--', 'LineWidth',1);
    max_range = max(max(abs(X(1:3,:)))) + 1;
    xlim([-max_range, max_range]); ylim([-max_range, max_range]); zlim([-0.2, max_range+1]);
    ...
    for k=1:3:N
        ...
        % body frame rotor positions
        arm = params.l;
        r1 = R * [ arm; 0; 0] + pos; % rotor 1
        r2 = R * [ 0; -arm; 0] + pos;
        r3 = R * [-arm; 0; 0] + pos;
        r4 = R * [ 0; arm; 0] + pos;

        % arms (x-symmetric and y-symmetric)
